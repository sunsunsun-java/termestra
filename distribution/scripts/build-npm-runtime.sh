...
case "$(uname -s)" in
  ...
  *) echo "Termestra runtime packages are supported only on macOS" >&2; exit 1 ;;
esac
...
mkdir -p "$package_dir"
cp -R "$source_dir/runtime" "$package_dir/"
mkdir -p "$package_dir/app"
"$(dirname "$0")/trim-macos-application.sh" \
  "$source_dir/app/termestra.jar" "$package_dir/app/termestra.jar" "$architecture"
