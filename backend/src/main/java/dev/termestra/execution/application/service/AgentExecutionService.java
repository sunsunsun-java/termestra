...
public final class AgentExecutionService implements AgentExecutionUseCase,AgentLaunchConfigurationQuery,AgentMessagingUseCase,RunOutputUseCase,AutoCloseable {
    ...
    private MessageDeliveryResult deliverCoordinated(String workspaceId,String workerId,String dispatchId,String senderName,String workerDescription,String text,String runtimePort){
        try{
            ...
            run.automaticInput.submit(AgentPromptBuilder.dispatch(senderName,workerDescription,dispatchId,text),true);
    ...
    @Override public MessageDeliveryResult report(String workspaceId,String workerName,String text,List<String> artifacts){return writeToAgent(workspaceId,workspaceId+":orchestrator",AgentPromptBuilder.report(workerName,text,artifacts),true);}
    ...
    private MessageDeliveryResult writeToAgent(String workspaceId,String agentId,String text){return writeToAgent(workspaceId,agentId,text,false);}
    private MessageDeliveryResult writeToAgent(String workspaceId,String agentId,String text,boolean deferIfBusy){return operations.withAgent(workspaceId,agentId,()->writeToAgentCoordinated(workspaceId,agentId,text,deferIfBusy));}
    private MessageDeliveryResult writeToAgentCoordinated(String workspaceId,String agentId,String text,boolean deferIfBusy){Optional<LiveRun> active=findActiveRun(workspaceId,agentId);if(active.isEmpty())return MessageDeliveryResult.failed("No active run for agent: "+agentId);if(!active.orElseThrow().ready())return MessageDeliveryResult.deferred("Agent startup is not complete; finish setup in its terminal before retrying.");try{requireAgent(workspaceId,agentId);active.orElseThrow().automaticInput.submit(text,deferIfBusy);return MessageDeliveryResult.success();}catch(InteractiveInputSubmitter.SubmissionException error){return deliveryFailure(error);}catch(RuntimeException error){return MessageDeliveryResult.failed(error.getMessage());}}
    private void deliverText(LiveRun run,String text){run.automaticInput.submit(text);}
    private long submitAutomaticInput(LiveRun run,String text,long readyAfterPosition,boolean deferIfBusy){
        ...
        try{
            ...
            return InteractiveInputSubmitter.submit(inputCommand(run),text,run::active,run.interactiveOutput::snapshot,writer,readyAfterPosition,deferIfBusy);
