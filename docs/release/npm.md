...
续传；若服务器不支持 Range，丢弃部分文件后重新完整下载，仍共享原截止时间、请求上限
和最终 SHA-512 校验。TLS 连接失败等没有进度的请求则等待 15 秒再试。边界耗尽时整个安装会明确失败，
...
PTY，并把每个 `.tgz` 限制在 75,000,000 bytes。启动参数 `--port` 只接受 0–65535
的十进制整数字符串（0 表示随机可用端口）；小数、十六进制和混合字符会明确报错。
...
5. `publish` job 先发布两个 runtime tarball，全部成功后才发布 CLI。每个包只有在
   ...
   工作流只会接受相同字节；不一致时必须发新版本。版本和 dist-tag 元数据请求连同
   JSON 响应体读取也受剩余验证时间约束，每次请求最多 30 秒；发布前的已存在版本查询
   使用 30 秒与配置验证时长的较小值，避免 registry 挂起使发布无限等待。
