...
export const Sidebar = ({
  ...
  return (
    <nav aria-label="Workspaces" className="flex h-full min-w-0 flex-col overflow-hidden">
      <div
        className="ws-sidebar-header flex items-center justify-between gap-2 pt-3 pr-10 pb-2 pl-3"
        style={{ boxShadow: 'inset 0 -1px 0 var(--border)' }}
      >
        <div className="flex min-w-0 items-center gap-2">
          <span
            className="ws-sidebar-title__text truncate text-xs font-medium text-ter"
      ...
      {workspaces === null ? (
        ...
        <div className="ws-empty flex min-h-0 min-w-0 flex-1 items-center overflow-hidden px-2 py-4">
          <EmptyState
            ...
            action={
              <button
                ...
                className="ws-empty__action icon-btn icon-btn--primary mt-1 flex min-w-0 items-center justify-center gap-1.5 px-4 py-2 text-xs font-medium disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Plus size={14} aria-hidden />
                <span className="ws-empty__action-label">{t('sidebar.newWorkspace')}</span>
              </button>
            }
          />
        </div>
              ...
              <li key={workspace.id} className="group relative">
                ...
                <Tooltip label={t('sidebar.deleteAria', { name: workspace.name })}>
                  <button
                    ...
                    className="ws-row-delete absolute flex h-6 w-6 items-center justify-center rounded text-ter opacity-0 transition-colors hover:text-status-red focus:opacity-100 group-hover:opacity-100"
                  >
          ...
          <li>
            <Tooltip label={createDisabledReason ?? t('sidebar.newWorkspace')}>
              <button
                ...
                className="ws-add ws-add--inline mt-2 flex items-center justify-start gap-2.5 border border-dashed py-1.5 pr-3 pl-2 text-xs font-medium text-sec transition-colors disabled:cursor-not-allowed disabled:opacity-50"
                style={{ borderColor: 'var(--border-bright)' }}
              >
                <span className="ws-add__icon flex h-8 w-8 shrink-0 items-center justify-center">
