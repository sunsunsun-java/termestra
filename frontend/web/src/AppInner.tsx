...
export const AppInner = () => {
  ...
  const wsCreate = useWorkspaceCreate({
    onWorkspaceCreated: (ws) => {
      setWorkspaces((current) => {
        if (current === null) return [ws]
        const existingIndex = current.findIndex((item) => item.id === ws.id)
        if (existingIndex < 0) return [...current, ws]
        return current.map((item, index) => (index === existingIndex ? ws : item))
      })
      selectWorkspace(ws.id)
      setWorkersByWorkspaceId((current) =>
        current[ws.id] ? current : { ...current, [ws.id]: [] }
      )
    },
    onWorkspaceExisting: (ws) =>
      toast.show({
        kind: 'warning',
        message: t('workspace.create.alreadyExists', { name: ws.name }),
      }),
  ...
  const terminalRunState = useTerminalRuns(eff.pollWorkspaceId)
  const terms = useOptimisticTerminalRuns(eff.pollWorkspaceId, terminalRunState.runs)
  ...
  const deleteWorkspace = useWorkspaceDelete({
    ...
    onWorkspaceDeleted: (workspaceId) => {
      wsCreate.forgetWorkspaceResult(workspaceId)
      terms.forgetOptimisticWorkspace(workspaceId)
    },
    selectWorkspace,
    setWorkersByWorkspaceId,
    setWorkspaces,
    ...
    <>
      ...
      >
        ...
        <AppOverlays
          ...
          taskGraphOpen={taskGraphOpen}
          tasksFile={tasksFile}
          connectionStale={tasksFile.connectionStale}
