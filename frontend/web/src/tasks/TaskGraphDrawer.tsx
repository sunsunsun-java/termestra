...
export const TaskGraphDrawer = ({
  ...
  const copyTaskLine = (lineIndex: number) => {
    ...
    void navigator.clipboard?.writeText(line).catch((error: unknown) => {
      console.error('[termestra] swallowed:tasks.copyLine', error)
    })
  }
  ...
  const filePath = workspacePath
    ? `${workspacePath}/.termestra/tasks.md`
    : '.termestra/tasks.md'
    ...
    >
      ...
      {connectionStale ? (
        <div
          ...
          style={{
            background: 'color-mix(in oklab, var(--status-orange) 8%, var(--bg-2))',
            borderColor: 'var(--border)',
            color: 'var(--status-orange)',
          }}
          role="status"
          data-testid="tasks-connection-stale"
        >
          <AlertTriangle className="mt-0.5 shrink-0" size={14} aria-hidden />
          <span>
            <strong className="font-medium">{t('runtime.staleTitle')}</strong>
            <span className="ml-1 text-ter">{t('runtime.staleBody')}</span>
          </span>
        </div>
      ) : null}
