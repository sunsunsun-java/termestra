...
export const WorkerCard = ({
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={readOnly ? undefined : () => onClick(worker)}
        disabled={readOnly}
        aria-label={t('worker.open', { name: worker.name })}
        className={`card worker-card relative flex w-full flex-col gap-3 overflow-hidden p-4 text-left${readOnly ? '' : ' card--interactive'}`}
      ...
      >
        ...
        <div className="flex min-w-0 flex-col gap-0.5">
          ...
          <span className="truncate text-xs leading-tight text-ter">
            {t(roleTranslationKey(worker.role))}
          </span>
        </div>
        <span
