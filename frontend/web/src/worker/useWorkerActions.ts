  ...
  const startWorkerAction = useCallback<WorkerActions['startWorker']>(
    async (workerId) => {
      ...
      try {
        ...
        // No optimistic status patch: the server derives working only when the
        // runtime is active and pending>0. The next listWorkers tick (≤500ms)
        // reconciles without inventing a transient idle/working state here.
